...
@section('page-subtitle', 'Perbarui informasi mitra yang sudah terdaftar.')
...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div class="px-8 py-6 border-b bg-slate-50/50 flex items-center gap-4">
                <div class="w-14 h-14 rounded-2xl bg-slate-100 border border-slate-200 p-1 flex items-center justify-center overflow-hidden shrink-0">
                    <img id="header_preview"
                         src="{{ $partner->logo_url }}"
                         alt="Logo {{ $partner->name }}"
                         class="w-full h-full object-contain">
                </div>
                <div>
                    <h2 class="font-black text-slate-800 text-lg">{{ $partner->name }}</h2>
                    <p class="text-xs text-slate-400">ID #{{ $partner->id }} • Ditambah {{ $partner->created_at->format('d M Y') }}</p>
                </div>
            </div>

            {{-- Form Body --}}
            <form action="{{ route('admin.partners.update', $partner) }}" method="POST" class="px-8 py-8 space-y-6">
                @csrf
                @method('PUT')
    ...
    <script>
        function updatePreview(url) {
            ...
            document.getElementById('header_preview').src = url;
        }
